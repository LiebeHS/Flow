...
- `js/components/config.js` hardcodes `API_URL` as an absolute LAN IP (currently `http://10.130.10.5:3000/api`, the machine that runs `npm start` — **not** `DB_HOST` in `.env`, which is a separate, dedicated MySQL server), not `localhost`, so coworkers on other machines can reach it. There's no env-based config for the frontend — if this machine's LAN IP changes (e.g. no DHCP reservation), this constant goes stale and every API call breaks silently in the console with 404s. Update it here if the backend host changes.
